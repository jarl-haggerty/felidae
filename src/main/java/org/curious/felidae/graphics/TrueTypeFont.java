...
public class TrueTypeFont {
	...
	public Rectangle.Float getStringVolume(String whatchars, float scaleX, float scaleY) {
		return getStringVolume(whatchars, 0, whatchars.length()-1, scaleX, scaleY, ALIGN_LEFT);
	}
	public Rectangle.Float getStringVolume(String whatchars, float scaleX, float scaleY, int format) {
		return getStringVolume(whatchars, 0, whatchars.length()-1, scaleX, scaleY, format);
	}
	
	public Rectangle.Float getStringVolume(String whatchars, int startIndex, int endIndex,
			float scaleX, float scaleY,
			int format
			) {
		Rectangle.Float result = new Rectangle.Float();
			...
			if( intObject != null ) {
				if (d < 0) totalwidth += (intObject.width-c) * d;
					if (charCurrent == '\n') {
						...
						result.add(new Rectangle.Float(totalwidth * scaleX, startY * scaleY, intObject.width * scaleX, intObject.height * scaleY));
						if (d > 0) totalwidth += (intObject.width-c) * d ;
					}
					i += d;
